...
## Milestone B runtime addendum — completed 2026-08-27

The first no-logistics continuity pass established that `TaskForceModeEnableTaskForceBuilder=False`, `TaskForceModeRepair=False`, and `TaskForceModeRearm=False` preserve surviving ships, their stable instance IDs, light damage, and expended ammunition across a full application restart and the following tactical mission.

The mission-specific unit allowlist did **not** prevent an already-owned Perry from deploying in Mission B-1. Although that node listed only the surviving Knox variants and disabled the builder, both the Knox and the excluded Perry appeared selected and entered the tactical mission. Therefore, do not treat a node allowlist as a proven deployment filter for owned persistent units. In this configuration it governs builder choices rather than persistent-force deployment, or is ignored when the builder is disabled. Restoring the Perry to the builder-enabled B-2 allowlist made its already-owned purchase row visible but greyed out.

Builder availability is independent of repair and rearm permission. Disabling the builder also removes access to its normal points display. This laboratory node intentionally tests a total builder lockout, but the user's Pacific Strike experience indicates that stock logistics-restricted transitions commonly keep the builder enabled with a deliberately restricted pool of inexpensive ships and selected aircraft even when repair and rearm are unavailable. Future campaign design should model those as separate decisions.

The full-logistics comparison confirmed different repair and rearm semantics. With repair enabled, each damaged owned ship gained a right-click `Repair` action in the builder. Repairing Light damage cost 10 points for the Knox and 21 points for the Perry, immediately changed both to `No Damage`, and reduced available points from 125 to 94. Rearm exposed no optional or purchasable menu action. Instead, with rearm enabled, all previously expended ammunition was automatically restored when Mission B-2 launched. A green `Rearmed` badge was runtime-confirmed in each selected ship's detail panel, but not on the compact task-force roster cards.

The builder permits one purchase of a given unit variant, not an arbitrary quantity of a class. A second Knox became purchasable only after `Variant2` was added alongside `Variant1`; there was no ordinary class-quantity field in the tested structures. This matches Pacific Strike's use of multiple numbered variants to offer multiple hulls from one class. The first purchased ship automatically became the starred flagship, and the player could later right-click another owned ship and choose `Assign Flagship`.

Mission A removed the destroyed Knox Variant2 and one of its four SH-2Fs from the current roster while preserving the surviving Knox, Perry, and three helicopters under their original persistent instance IDs. Both ships carried Light damage and expended stores through save, full application restart, reload, and Mission B-1. The no-logistics mission added +5 points/+5 cap, producing 125/525. After 31 points of explicit repairs, the full-logistics mission automatically rearmed the force and added +5/+5, ending at 99 points and 530 cap. Per-node completion reward guards remained set, and the arithmetic proves the earlier +20/+20 reward was not duplicated after reload.

Milestone B's core exit gate is satisfied for roster carry-over, losses, ammunition, damage, repair, rearm, points, cap, rewards, and save/reload. Granted-unit and detached/subset behavior were optional and remain untested. A mission allowlist should not be used as the sole mechanism for preventing an already-owned unit from deploying.

### Stock-campaign runtime observations supplied after Milestone B

The user confirmed from prior Pacific Strike play that optional lettered missions can create both economic rewards and persistent tactical advantages. In optional submarine mission `08A Pathfinders`, scouting the land targets causes them to be immediately visible in the following mission; skipping the reconnaissance leaves them subject to normal detection. Stock files corroborate the mechanism: `08A Pathfinders.ini` sets `08APalawanReconCompleted=True`, and `09 Shadows off Palawan.ini` checks that variable and executes `Action_UnitRevealToTaskforce=Taskforce1|Identify` with unlimited reveal time.

The user likewise observed that destroying an enemy task group in an earlier persistent-campaign mission can prevent it from appearing later. Pacific Strike provides a concrete authored example: optional `07A Hunt for the Cruiser` sets `07ASlavaDestroyed=True`, while units in `10 Vengeance at Luzon` use `SpawnByVariableAND=07ASlavaDestroyed,IsFalse`. This confirms persistent flags as the appropriate stock pattern for threat removal and intelligence advantages across missions.

Pacific Strike also runtime-confirms deployment-count limits. One optional mission allowed only one roster checkmark before all other ships greyed out, and another capped deployment at three ships. The stock campaign fields corroborate this UI behavior: `TaskForceModeRequiredUnitType=Vessel` paired with `TaskForceModeMaxUnits=1` and `TaskForceModeMaxUnits=3`. This is the correct mechanism for lone-ship or limited-surface-force selection; mission allowlists should determine eligible choices, while `TaskForceModeMaxUnits` limits how many eligible owned units may deploy.

These stock observations reduce—but do not replace—the optional clean-room detached-force test: the remaining question is limited to whether undeployed units' exact damage and ammunition state remain byte-for-byte unchanged across a custom detached mission. The broader ability to select a one- or three-vessel subset and to carry optional-mission consequences forward is established.
